#!/usr/bin/env perl
# A simple perl wrapper for AcroPoly to be able to specify the contig name and maximum genotype missing rate
# for haplotyping.
...
my %options_set;
my @to_remove;
my @iftrue_opts = ("v", "verbose", "d", "deterministic", "rel", "relative", "expression");
my @toget_opts = ("c", "contig", "l", "length", "s", "shift", "iter", "itermax", "cov", "mincov", "eps", "epsrel", "mmq", "mbq", "maxIS", "maxMISS");
my $n=-1;
...
for (my $i=0; $i<10; $i+=2){ # check doubly given value getting options
...
for (my $i=0; $i<6; $i+=2){ # check doubly given set true options 
...
my $break_vcf = !system("${AcroPoly_Path}/break_vcf.sh ${contig_vcf} ${contig_vcf_broken}"); # break complex variants and throw out indels using break_vcf.py
$break_vcf or die "Couldn't break the complex variants and throw out indels from the VCF file: $!\n";
$ARGV[1] = $contig_vcf_broken;

if (exists $options_set{contig}) {delete $options_set{contig}} # not passed to AcroPoly.py
if (exists $options_set{maxMISS}) {delete $options_set{maxMISS}} # not passed to AcroPoly.py
...
undef %options_set;
my $phasing = system((join " ","${AcroPoly_Path}/AcroPoly.py", @ARGV)." ".(join " ", map { $new_options_set{$_} eq "" ? "$_" : "$_ $new_options_set{$_}" } (keys %new_options_set))); 
!$phasing or die "ERROR: failed to run AcroPoly: $!\n";
